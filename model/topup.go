...
// RechargeWaffoPancake handles Pancake-side order.completed events. Same
// row-locked, idempotent shape as RechargeWaffo — kept separate so the two
// integrations can drift independently (Pancake uses dynamic priceSnapshot
// and a different webhook flow from the legacy Waffo SDK).
func RechargeWaffoPancake(tradeNo string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("Waffo Pancake 充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
		MaybeGrantInviteRewardOnFirstTopUp(topUp.UserId)
	}
